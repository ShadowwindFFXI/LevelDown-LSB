...
function controller.onMinionDeath(mob)
    ...
    -- Increment minion counter
    local minions = zone:getLocalVar("DI_MINION") + 1
    zone:setLocalVar("DI_MINION", minions)

    -- Schedule dragon spawn 2 minutes after 11th minion dies
    if minions >= 11 and zone:getLocalVar("DI_WYVERN_SPAWNED") == 0 then
        local now = GetSystemTime()
        SetServerVariable("DI_DRAGON_SPAWN_TIME_"..zone:getID(), now + controller.dragonDelay)
        broadcast("The Domain Invasion dragon will arrive shortly!") --Not Used in retail
        -- broadcast("The Domain Invasion dragon will arrive shortly!") --Not Used in retail
    end
end
...
function controller.handleDespawn(zoneId)
    ...
    if mireu and mireu:isSpawned() then
        activeMob = mireu
        DespawnMob(mireuId)
    elseif mob and mob:isSpawned() then
        activeMob = mob
        DespawnMob(wyrmId)
    else
        -- fallback (minions case)
        activeMob = mob or mireu
    end
