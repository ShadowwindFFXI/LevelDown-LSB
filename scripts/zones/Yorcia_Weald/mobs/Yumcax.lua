...
entity.onMobWeaponSkill = function(target, mob, skill)
    ...
    if skillId == 3059 then -- uproot
        if preventRepeat == 1 then
            return
        end

        for _, effect in ipairs({ xi.effect.PARALYSIS, xi.effect.RASP }) do
...
entity.onWeaponskillHit = function(mob, attacker, weaponskill)
    if attacker and
        ...
        if weaponskill == xi.weaponskill.EARTH_CRUSHER then
...
entity.onMobDeath = function(mob, player, optParams)
    if player then
        player:addTitle(xi.title.YUMCAX_LOGGER)
    end
end
