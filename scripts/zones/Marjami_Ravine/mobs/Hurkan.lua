...
entity.onMobWeaponSkill = function(target, mob, skill)
    ...
    if skillId == 3072 then -- crashing thunder
        if preventRepeat == 1 then
            return
        end

        for _, effect in ipairs({ xi.effect.SILENCE, xi.effect.RASP }) do
        ...
        mob:addMod(xi.mod.EARTH_ABSORB, 100)
...
entity.onWeaponskillHit = function(mob, attacker, weaponskill)
    if attacker and
        ...
        if weaponskill == xi.weaponskill.PENTA_THRUST then
...
entity.onMobDisengage = function(mob)
    mob:timer(30000, function(mobArg)
        ...
        if  mobArg:getLocalVar("PreventRepeatEffect") == 1 then
            ...
            mobArg:delStatusEffect(xi.effect.SILENCE)
            mobArg:delStatusEffect(xi.effect.RASP)
        end
    end)
end
...
entity.onMobDeath = function(mob, player, optParams)
    if player then
        player:addTitle(xi.title.HURKAN_BIRDLIMEIST)
    end
end
