...
entity.onMobWeaponSkill = function(target, mob, skill)
    ...
    if skillId == 3137 then -- polar roar
        if preventRepeat == 1 then
            return
        end

        for _, effect in ipairs({ xi.effect.PARALYSIS, xi.effect.FROST }) do
...
entity.onWeaponskillHit = function(mob, attacker, weaponskill)
    if attacker and
        ...
        if weaponskill == xi.weaponskill.SWIFT_BLADE then
...
entity.onMobDisengage = function(mob)
    mob:timer(30000, function(mobArg)
        ...
        if  mobArg:getLocalVar("PreventRepeatEffect") == 1 then
            ...
            mobArg:delStatusEffect(xi.effect.PARALYSIS)
            mobArg:delStatusEffect(xi.effect.FROST)
        end
    end)
end
...
entity.onMobDeath = function(mob, player, optParams)
    if player then
        player:addTitle(xi.title.KUMHAU_ROASTER)
    end
end
