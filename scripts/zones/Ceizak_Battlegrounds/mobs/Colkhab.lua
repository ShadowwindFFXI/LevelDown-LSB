...
entity.onMobWeaponSkill = function(target, mob, skill)
    ...
    if skillId >= 3002 and skillId <= 3007 then
        mob:addStatusEffect(xi.effect.BLINK, { power = 3, duration = 30, origin = mob })
        mob:messageBasic(xi.msg.basic.GAINS_EFFECT_OF_STATUS, xi.effect.BLINK)
    end

    if skillId == 3005 then -- Droning whirlwind
        if preventRepeat == 1 then
            return
        end

        for _, effect in ipairs({ xi.effect.AMNESIA, xi.effect.CHOKE }) do
...
entity.onWeaponskillHit = function(mob, attacker, weaponskill)
    if attacker and
        ...
        if weaponskill == xi.weaponskill.AEOLIAN_EDGE then
            mob:delStatusEffect(xi.effect.ICE_SPIKES)
            mob:delStatusEffect(xi.effect.AMNESIA)
...
entity.onMobDisengage = function(mob)
    mob:timer(30000, function(mobArg)
        ...
        if  mobArg:getLocalVar("PreventRepeatEffect") == 1 then
            ...
            mobArg:delStatusEffect(xi.effect.ICE_SPIKES)
            mobArg:delStatusEffect(xi.effect.AMNESIA)
            mobArg:delStatusEffect(xi.effect.CHOKE)
        end
    end)
end
...
entity.onMobDeath = function(mob, player, optParams)
    if player then
        player:addTitle(xi.title.COLKHAB_DETHRONER)
    end
end
