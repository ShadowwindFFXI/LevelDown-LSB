-----------------------------------
-- Omixi Util
-- Technically not enum but required preloaded for base modules
-----------------------------------
require("modules/module_utils")
-----------------------------------
local m = Module:new("omixi_util")

omixi      = omixi or {}
omixi.util = omixi.util or {}

local pathWithFilename = io.popen("cd"):read'*all'

omixi.util.ensureNPC = function(zoneName, npcName)
...
omixi.util.removeNPC = function(npc)
...
omixi.util.createLookup = function(tbl)
    local result = {}
...
omixi.util.forAlliance = function(player, func)
    if
        player == nil or
        not player:isPC()
    then
...
-- eg. omixi.util.randomNoRepeat(5, 3) randomly returns 1,2,4,5
omixi.util.randomNoRepeat = function(size, exclude)
    local range = {}
...
omixi.util.randomBitNoRepeat = function(bits, num)
    local range = {}
...
omixi.util.setMusic = function(player, id)
...
omixi.util.helperScroll = function(player, qty)
    if
...
omixi.util.printList = function(tbl)
    local result = tbl[1]
...
omixi.util.tradeContainsOnly = function(trade, list)
...
omixi.util.tradeHasExactly = function(trade, items)
...
omixi.util.giveItem = function(player, items, params)
    params = params or {}
...
omixi.util.spawnEncounter = function(player, npc, info)
    ...
    if info.mobs ~= nil then
        for _, mobInfo in pairs(info.mobs) do
            ...
            local mobTable =
            {
                ...
                minLevel    = mobInfo.level or 1,
                maxLevel    = mobInfo.level or 1,
...
omixi.util.mobEncounter = function(mob, params)
    setEncounter(mob, params)
end

omixi.util.allianceEncounter = function(player, params)
    local zoneID   = player:getZoneID()
    local alliance = player:getAlliance()
...
omixi.util.leaveEncounter = function(player)
...
omixi.util.finishEncounter = function(player)
    local zoneID   = player:getZoneID()
    local alliance = player:getAlliance()
...
omixi.util.scaledEncounter = function(player, mob, bonus)
    local zoneID   = player:getZoneID()
    local alliance = player:getAlliance()
    local total    = 0
...
-----------------------------------
omixi.util.addDynamicMob = function(zone, mobInfo)
    -- Extract the numbers from the table before building the entity
    local minLvl = type(mobInfo.level) == "table" and mobInfo.level[1] or mobInfo.level
    local maxLvl = type(mobInfo.level) == "table" and mobInfo.level[2] or mobInfo.level
    ...
    local dynamicEntity =
    {
        ...
        -- Ensure these are numbers, not the 'level' table itself
        minLevel    = minLvl or 1,
        maxLevel    = maxLvl or 1,
    ...
    dynamicEntity.onMobInitialize = function(mob)
        ...
        -- Randomly select a level from the range table to pass a single number
        mob:setMobLevel(math.random(mobInfo.level[1], mobInfo.level[2]))
        else
            -- Already a number, pass as-is
            mob:setMobLevel(mobInfo.level)
        end
...
-- String Utils
-----------------------------------
omixi.util.capitalize = function(str)
    return string.upper(string.sub(str, 1, 1)) .. string.lower(string.sub(str, 2))
end

omixi.util.numWithCommas = function(n)
...
omixi.util.nextVanaWeek = function()
...
omixi.util.simpleMenu = function(player, npc, tbl, func, title, currentPage, param)
    local options  = {}
    local max      = 1
    ...
    then
        ...
        table.insert(options, {
            "(Prev)",
            function(player)
                omixi.util.simpleMenu(player, npc, tbl, func, title, gotoPage, param)
            end,
        })
    end
...
omixi.util.simpleShop = function(player, npc, tbl, func, title, currentPage, param)
    local options  = {}
    local max      = 1
    local lastPage = math.floor((#tbl - 1) / 4)
    local page     = currentPage
    ...
    if page > 0 then
        table.insert(options, {
            "(Prev)",
            function(player)
                omixi.util.simpleShop(player, npc, tbl, func, title, page - 1, param)
            end,
        })
    end
...
omixi.util.categoryMenu = function(player, npc, tbl, func, title)
    local options = {}
        ...
        table.insert(options, {
            ...
            function()
                if type(category[2]) == "table" then
                    omixi.util.simpleShop(player, npc, category[2], func, title)
                else
...
omixi.util.skillMenu = function(player, npc, tbl, func, title)
    local options = {}
    ...
    for _, category in pairs(tbl) do
        table.insert(options, {
            ...
            function()
                if type(category[2]) == "table" then
                    omixi.util.simpleMenu(player, npc, category[2], func, title, 0, { skill = true })
                else
                    category[2](player, npc)
                end
            end,
...
-----------------------------------
-- { { rate, item } }, modifier
omixi.util.pickItem = function(items, mod)
    -- sum weights
    local sum = 0
    for i = 1, #items do
        if
...
omixi.util.treasurePool = function(player, pools, source) -- source optional
...
omixi.util.openChest = function(npc)
...
-----------------------------------
omixi.util.decToLE = function(num)
...
omixi.util.look = function(tbl)
    ...
    for k, v in pairs(modelSlot) do
        if tbl[v] then
            str = str .. omixi.util.decToLE(tbl[v])
        else
            str = str .. "0000"
        end
    end
...
omixi.util.dialogDelay = function(tbl)
...
omixi.util.dialog = function(player, tbl, npcName, param)
    if
        type(tbl) ~= "table" and
        type(tbl) ~= "function"
    then
...
-- Override Utils
-----------------------------------
omixi.util.overrideDuplicate = function(entity, event, func)
    if not entity then
        return
    end
...
omixi.util.liveReload = function(mod, tbl)
        ...
        for _, entity in pairs(entityList) do
            mod:addOverride(string.format("xi.zones.%s.Zone.onInitialize", zoneName), function(zone)
             super(zone)   
                ...
                if entity.objtype == xi.objType.MOB and entity.minLevel == nil then
                    if type(entity.level) == "table" then
                        entity.minLevel = entity.level[1]
                        entity.maxLevel = entity.level[2]
                    else
                        entity.minLevel = entity.level or 1
                        entity.maxLevel = entity.level or 1
                    end
                end
            ...
            then
                ...
                if de ~= nil then
                    ...
                    local zone = GetZone(omixi.zone[zoneName])
...
omixi.util.reloadOverride = function(mod, str, func)
...
omixi.util.npcSpawner = function(tbl, npc, spawner, keepSpawner)
    local result = {}
    local before = {}
    local after  = {}
...
omixi.util.addSkillEffect = function(mod, skillName, mobName, effect)
...
omixi.util.ensureMob = function(zoneName, mobName)
